...
const FormInput = ({type, label, placeholder, fieldRef, hasError}: Props) => {
    ...
    const inputStyle = 'form-control w-full px-3 py-1.5 text-gray-700 rounded border border-solid border-slate-300 focus:border-slate-800 focus:outline-none'

  return (
    <div className='form-group mb-6'>
        <label className='black text-slate-800 text-sm font-bold mb-2'>{label}</label>
        {type === 'textarea' ? (
            <textarea className={inputStyle} rows={3} placeholder={placeholder} {...fieldRef} />
        ) : (
